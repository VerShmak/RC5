...
Подготовка ключа:
Для начала на вход поступает целове число, с помощью которого генерируется секретный ключ, длина которого в байтах равна данному числу. Ключ представляет из себя специальный набор чисел, который будет использоваться для шифрования.
